...
$comment = $db->getPostCommentById($data['idcomment']);
...
if($user->getId()>0){
    if($comment->getId()!=0 && strlen($data['replycomment'])!=0 && $subcomment->getId()==0){
        $comment_id = $db->insertReplyPostComment($data['id_user'],$comment->getIdPost(),$data['idcomment'],$comment->getIdUser(),$data['replycomment'],$user_comment->getLogin(),$comment->getText());
        $replycomment = $db->getPostCommentById($comment_id);
        ...
        $comment_id = $db->insertReplyPostComment($data['id_user'],$comment->getIdPost(),$data['idcomment'],$subcomment->getIdUser(),$data['replycomment'],$user_subcomment->getLogin(),$subcomment->getText());
        $replycomment = $db->getPostCommentById($comment_id);
        if($replycomment->getId()!=0){
            $retval['saved'] = 'yes';
            $retval['idreplycomment'] = $replycomment->getId();
