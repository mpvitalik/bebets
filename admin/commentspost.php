...
$comments = $db->getAllCommentsPostAdmin($page);
$pages_result = $db->pagesResultCommentsPost();
...
<div class="container">
    ...
    <table class="table">
        ...
        <?php foreach($comments as $comment):?>
            ...
            $comment_post = $db->getPostById($comment->getIdPost())
            ?>
            <tr>
                ...
                <td><a href="../<?=$comment_post->getSsilka()?>.html"><?=$comment_post->getNazva()?></a></td>
                <td>
                    <a href="deletecommentpost.php?id=<?=$comment->getId();?>">Удалить</a>
                </td>
            </tr>
        <?php endforeach;?>
        </tbody>
    ...
    <div style="border-top: 1px dashed grey;padding:10px;">
        <?php if($pages != 0):?>
            <?php if($page!=1):?>
                <a href="commentspost.php?p=<?=$page-1;?>">назад</a>
            <?php endif;?>
            <?=$page;?> из <?=$pages;?>
            <?php if($page!=$pages):?>
                <a href="commentspost.php?p=<?=$page+1;?>">вперед</a>
            <?php endif;?>
        <?php endif;?>
    </div>
...
<script>
    ...
    $(".deletesomeitems").click(function(){
        ...
        $.post('someitems.php',{ids:deletelist_json,action:"delete",type:"postcomments"},function(data){
            if(data=="ok"){
                window.location.href = "commentspost.php";
            }
        });
    });
