...
if($user->getId()>0){
    $user_likedislike_this_comment_early = $db->getPostCommentUserLikeDislike($data['id_user'],$data['idcomment']);
    if($user_likedislike_this_comment_early->getId()!=0){
        if($data['datatype']=="like" || $data['datatype']=="dislike"){
            $db->updatePostCommentLikeDislike($data['id_user'],$data['idcomment'],$data['datatype']);
        }
    }else{
        if($data['datatype']=="like" || $data['datatype']=="dislike"){
            $db->insertPostCommentLikeDislike($data['id_user'],$data['idcomment'],$data['datatype']);
        }
    }
}

$comment_likesdislikes = $db->getPostCommentLikesDislikes($data['idcomment']);
